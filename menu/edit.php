...
session_start();
  if(isset($_SESSION["username"]) && isset($_SESSION["company_id"])){
            if(isset($_GET['id'])){
                        $sql = "SELECT menu_title,id,display_in_menu,menu_url,menu_parent_id,status,menu_icon FROM tbl_menu where id=".$_GET['id'];
                        $allmenus = mysqli_query($conn, $sql);
                        $rowcount = mysqli_num_rows($allmenus);
                        if($rowcount > 0){
                                $menuByid = mysqli_fetch_array($allmenus);
			...
			if(isset($_POST['update'])){
				...
				$updatedon			 = date("U");
				$sql    = "UPDATE tbl_menu SET display_in_menu= '$display_in_menu', menu_parent_id= '$menu_parent_id', menu_title= '$menu_title', menu_url= '$menu_url', menu_icon= '$menu_icon', status= '$status', updatedby= '$updatedby',  updatedon= '$updatedon'  WHERE id=".$_POST["hid"];
						   
				$result =  mysqli_query($conn,$sql);
				// var_dump($result);exit;
				if($result){
					echo "<script type='text/javascript'>alert('Data Saved Successfully'); window.history.go(-2);</script>";
				}else{
                                    ...
                                    <div class="col-lg-6">
                                            <div class="card shadow">
												...
												<form  method="post" action="edit.php?id=<?=$_GET['id'];?>">
													<input name="hid" value="<?=$_GET['id'];?>" hidden>
													<div class="custom-control custom-checkbox mb-3">
														<input class="custom-control-input checked" name="display_in_menu" value="1" id="customCheck1" type="checkbox" <?php echo ($menuByid['display_in_menu']== 1)? 'checked':'';?>>
															 ...
															 <?php $sql = "SELECT menu_title,id,menu_parent_id FROM tbl_menu";
															  $result = mysqli_query($conn, $sql);
																while($row = mysqli_fetch_array($result)){
																		?> <option value="<?php echo $row['id'];?>" <?php echo ($menuByid['menu_parent_id'] == $row['id'])? 'selected':'';?> > <?php echo $row['menu_title'];?></option> <?php
																}
															  ?>
                                                        </select>
                                                    </div>
                                                    <div class="form-group row">
                                                        ...
                                                        <input type="text" class="form-control" name="menu_title" value="<?=$menuByid['menu_title'];?>"  placeholder="Dashboard" required>
                                                    </div>
													<div class="form-group row">
                                                        <label class="form-label">Menu url</label>
                                                        <div class="input-group">
                                                            <input type="text"  name="menu_url" class="form-control" value="<?=$menuByid['menu_url'];?>" placeholder="eg. menu-add" required>
                                                        </div>
                                                    </div>
													<div class="form-group row">
                                                        <label class="form-label">Menu icon</label>
                                                        <input type="text" class="form-control" name="menu_icon" value="<?=$menuByid['menu_icon'];?>" placeholder="fa fa-home" required>
                                                    </div>
                                                    
													<div class="form-group row">
                                                        <label class="form-label">Status</label>
                                                        <select name="status" id="select-countries" class="form-control custom-select" >
                                                            <option value="1" <?php echo ($menuByid['status'] == 1)? 'selected':'';?>>Active</option>
															<option value="0" <?php echo ($menuByid['status'] == 0)? 'selected':'';?>>Inactive</option>
                                                        </select>
                                                    </div>
                                                   
													<button type="submit" name="update" class="btn btn-primary float-right" value="update">Update</button>
												</form>
                                                </div>
                                            </div>
                                    </div>
